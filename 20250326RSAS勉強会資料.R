...
expected <- survfit(Surv(t_test, d_test) ~ strata(strata_test), data = df_test)
print(expected$surv)
print(expected$time)
...
Rcpp::List calculateKaplanMeier_rcpp(Rcpp::NumericVector t, Rcpp::IntegerVector d, Rcpp::NumericVector w,
  ...
  if (strata.size() == 0 || Rcpp::unique(strata).size() == 1) {
    ...
    for (int i = 0; i < u; ++i) {
      ...
      double sum_events = 0;
      double sum_risk = weighted_n_risk[i] * (weighted_n_risk[i]);
      if (sum_risk > 0) {
        std_err[i] = sqrt(weighted_events / sum_risk);
      } else {
        std_err[i] = NA_REAL;
      }
    }
...
km_list.push_back(km_object);
  ...
  for (int i = 0; i < Rcpp::max(strata_vec); ++i) {
    ...
    for (int j = 0; j < u; ++j) {
      ...
      double sum_events = 0;
      double sum_risk = weighted_n_risk[j] * (weighted_n_risk[j]);
      if (sum_risk > 0) {
        std_err[j] = sqrt(weighted_events / sum_risk);
      } else {
        std_err[j] = NA_REAL;
      }
    }
