reporting_competing_risk <- function(nuisance.model, exposure, effect.modifier, estimand, alpha_beta_estimated, cov_estimated,
  ...
  rr_1 <- exp(alpha_beta_estimated[i_parameter[3]])
  rr_2 <- exp(alpha_beta_estimated[i_parameter[7]])
  se_1 <- sqrt(cov_estimated[i_parameter[3], i_parameter[3]])
  se_2 <- sqrt(cov_estimated[i_parameter[7], i_parameter[7]])
  rr_1_l <- exp(alpha_beta_estimated[i_parameter[3]] - critical_value * se_1)
  rr_1_u <- exp(alpha_beta_estimated[i_parameter[3]] + critical_value * se_1)
  rr_2_l <- exp(alpha_beta_estimated[i_parameter[7]] - critical_value * se_2)
  rr_2_u <- exp(alpha_beta_estimated[i_parameter[7]] + critical_value * se_2)
  z_1 <- abs(alpha_beta_estimated[i_parameter[3]]) / se_1
  z_2 <- abs(alpha_beta_estimated[i_parameter[7]]) / se_2
  p_1 <- 2 * (1 - pnorm(z_1))
  p_2 <- 2 * (1 - pnorm(z_2))
