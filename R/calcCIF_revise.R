...
calcAalenVariance <- function(
    ...
    CIF_value,
    n.event,
    #n.event2は、同時刻に2種のイベントが起きないと仮定すれば不要
    n.atrisk,
    km_time,
    km_value
){
  ...
  third_cum <- 0
  for(i in 1:n){
    #index <- min(length(CIF_value)-1, sum(as.numeric((km_time-CIF_time[i])<0))) # 元のミス
    #index <- sum(km_time<CIF_time[i]) # 元の単純化
    index <- sum(CIF_time<km_time[i]) # 正しいはず(逆になっていた)
    if(index!=0){
      # Y_lの項n.atriskについてはCIFの値同様indexを用いるのが正しいはず
      # third_termの分母のdは同時刻に2種のイベントが起きない限りはこれでよいはず
      first_cum <- first_cum + ((CIF_value[index+1]-CIF_value[i])^2)*n.event[i]/(n.atrisk[index+1]-1)/(n.atrisk[index+1]-n.event[i])
      second_cum <- second_cum + ((km_value[index]^2)*(n.event[i])*(n.atrisk[index+1]-n.event[i])/(n.atrisk[index+1]^2)/(n.atrisk[index+1]-1))
      third_cum <- third_cum + ((CIF_value[index+1]-CIF_value[i])*km_value[index]*n.event[i]*(n.atrisk[index+1]-n.event[i])/n.atrisk[index+1]/(n.atrisk[index+1]-n.event[i]))/(n.atrisk[index+1]-1)
    }
...
calcDeltaVariance <- function(
  ...
  for(i in 1:n){
    ...
    if(index!=0){
      ...
      # Y_lの項n.atriskについてはCIFの値同様indexを用いるのが正しいはず
      first_cum <- first_cum + ((CIF_value[index+1]-CIF_value[i])^2)*n.event[i]/(n.atrisk[index+1]-1)/(n.atrisk[index+1]-n.event[i])
      second_cum <- second_cum + ((km_value[index]^2)*(n.event[i])*(n.atrisk[index+1]-n.event[i])/(n.atrisk[index+1]^3))
      third_cum <- third_cum + ((CIF_value[index+1]-CIF_value[i])*km_value[index]*n.event[i]/(n.atrisk[index+1]^3))
    }
    first_term[i] <- first_cum
    second_term[i] <- second_cum
    third_term[i] <- third_cum
...
calcCIF_H(nuisance.model = model3, exposure = 'fruitq1',
          data = jdcs, devideByExposure = TRUE, drawGraph = TRUE, drawGraphCI = TRUE, var_method = "Delta")
