predict.polyreg_ <- function(
  ...
  for (i_x in seq_len(nrow(x_l))) {
    ...
    if (!is.null(initial_pred)) {
      log_p_ <- log(initial_pred[i_x, ])
    } else {
      log_p_ <- log(c(0.5, 0.5, 0.5, 0.5))
    }
    ...
    if (optim.method$inner.optim.method == 'optim' | optim.method$inner.optim.method == 'BFGS') {
      ...
      sol <- optim(par = log_p_, fn = eq_fn, method = "BFGS", control = list(maxit = optim.method$optim.parameter8, reltol = optim.method$optim.parameter7))
